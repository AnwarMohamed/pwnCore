...
public class MainService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		sendBroadcast(new Intent().setAction(StaticClass.PWNCORE_SERVICE_STARTED));
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					if (client.login(con_txtUsername, con_txtPassword)) {
						...
						tmpIntent
								.setAction(StaticClass.PWNCORE_CONNECTION_SUCCESS);
	...
	public BroadcastReceiver mainReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, final Intent intent) {
			...
			else if (action == StaticClass.PWNCORE_LOAD_ALL_MODULES
					&& isAuthenticated) {

				Intent tmpIntent = new Intent();
				tmpIntent
						.setAction(StaticClass.PWNCORE_DATABASE_UPDATE_STARTED);
				sendBroadcast(tmpIntent);

				String[] modules = { StaticClass.PWNCORE_LOAD_EXPLOITS,
			...
			} else if (action == StaticClass.PWNCORE_LOAD_EXPLOITS
					&& isAuthenticated) {
			...
			} else if (action == StaticClass.PWNCORE_LOAD_NOPS
					&& isAuthenticated) {
				if (isAuthenticated) {
					executor.execute(new Thread() {
						@Override
			...
			} else if (action == StaticClass.PWNCORE_LOAD_ENCODERS
					&& isAuthenticated) {
				executor.execute(new Thread() {
					@Override
					public void run() {
