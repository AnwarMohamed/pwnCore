...
public class NmapXmlParser {
		...
		try {		
			...
			for (int s = 0; s < nodeLst.getLength(); s++) {
				...
				if (fstNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					NodeList hostElementList = fstElmnt.getElementsByTagName("host");
	
					for (int i=0; i < hostElementList.getLength(); i++) {	
						mHostItems.add(new HostItem());
						getHost((Element)hostElementList.item(i), mHostItems.size() - 1);
	                } 
				}
			}
		}
	...
	private void getHost(Element e, int index) {				
		if (e.hasChildNodes()) {		
			...
			for (int i=0; i<hostNodeList.getLength(); i++) {
				...
				if (hostNodeList.item(i).getNodeName().equals("status")) {
				...
				else if (hostNodeList.item(i).getNodeName().equals("hostnames")) {
					if (hostNodeList.item(i).getNodeType() == Node.ELEMENT_NODE) {
						
						Element hostnameElement = (Element)hostNodeList.item(i);
						NodeList hostnameElementList = hostnameElement.getElementsByTagName("hostname");

						for (int j=0; j< hostnameElementList.getLength(); j++) {		
							getHostHostnames((Element)hostnameElementList.item(j), index);
		                } 			
					}
				}	
					...
					if (hostNodeList.item(i).getNodeType() == Node.ELEMENT_NODE) {
						...
						NodeList portElementList = portElement.getElementsByTagName("port");

						for (int j=0; j< portElementList.getLength(); j++) {		
							getHostPorts((Element)portElementList.item(j), index);
		                } 			
					}
				}	
				...
				else if (hostNodeList.item(i).getNodeName().equals("distance")) {
					
				}	
				else if (hostNodeList.item(i).getNodeName().equals("tcpsequence")) {
					
				}	
				else if (hostNodeList.item(i).getNodeName().equals("ipidsequence")) {
					
				}	
				else if (hostNodeList.item(i).getNodeName().equals("tcptssequence")) {
	...
	private void getHostPorts(Node n, int index) {
		...
		tmp.Protocol = n.getAttributes().getNamedItem("protocol").getNodeValue();
	...
	private void getHostAddress(Node n, int index) {
		...
		tmp.AddressType = n.getAttributes().getNamedItem("addrtype").getNodeValue();
	...
	private void getHostStatus(Node n, int index) {
		...
		tmp.StateReason = n.getAttributes().getNamedItem("reason").getNodeValue();
	}
